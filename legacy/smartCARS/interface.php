...
require_once(__DIR__.'/../codon.config.php');
require_once(__DIR__.'/../local.config.php');
require_once(__DIR__."/../common/NavData.class.php");
require_once(__DIR__."/../common/ACARSData.class.php");
...
class smartCARS {
	...
	static function manuallogin($userid, $password, $sessionid) {
		...
		$ret = array();
		$user = $userid;
		$param = "SELECT * FROM " . TABLE_VAOS . "users WHERE ";
	...
	static function automaticlogin($dbid, $oldsessionid, $sessionid) {
		...
		$stmt->closeCursor();
		if($res1['dbid'] != "") {
			$stmt = $dbConnection->prepare("SELECT * FROM " . TABLE_VAOS . "users WHERE id = ?");
	...
	static function getairports($dbid) {
		global $dbConnection;
		$ret = array();
		$stmt = $dbConnection->prepare("SELECT * FROM " . TABLE_VAOS . "airports ORDER BY icao");
		$stmt->execute();
	...
	static function getaircraft($dbid) {
		global $dbConnection;
		$res = array();
		$stmt = $dbConnection->prepare("SELECT * FROM " . TABLE_VAOS . "aircraft ORDER BY name");
		$stmt->execute();
