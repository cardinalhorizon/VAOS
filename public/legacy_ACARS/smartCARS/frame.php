...
switch ($action) {
    ...
    case "getaircraft":
        ...
        foreach ($res['aircraft'] as $ac) {
            ...
            $ac = str_replace(";", "", $ac);
            $ac = str_replace(",", "", $ac);
            echo($ac[$res['format']['id']] . "," . $ac[$res['format']['fullname']] . "," . $ac[$res['format']['icao']] . "," . $ac[$res['format']['registration']] . "," . $ac[$res['format']['maxpassengers']] . "," . $ac[$res['format']['maxcargo']] . "," . "0"/*$ac[$res['format']['requiredranklevel']]*/);
            $runcount++;
        }
        break;
    case "getbidflights":
        ...
        if (sizeof($res) > 0) {
            ...
            foreach ($res as $schedule) {
                ...
                echo($schedule[$res['format']['bidid']]
                    . "|" . $schedule['routeid']
                    . "|" . $schedule['code']
                    . "|" . $schedule['flightnumber']
                    . "|" . $schedule['departureicao']
                    . "|" . $schedule['arrivalicao']
                    . "|" . $schedule['route']
                    . "|" . $schedule['cruisingaltitude']
                    . "|" . $schedule['aircraft']
                    . "|" . $schedule['duration']
    ...
    case "createflight":
        if (check_session($_GET['dbid'], $_GET['sessionid']) == true) {
            $ret = smartCARS::createflight($_GET['dbid'], $_GET['flightnumber'], $_GET['departureicao'], $_GET['arrivalicao'], $_GET['aircraft'], $_GET['flighttype'], $_GET['departuretime'], $_GET['arrivaltime'], $_GET['flighttime'], $_POST['route'], $_GET['cruisealtitude'], $_GET['distance']);
            if ($ret == true) {
                echo("SUCCESS");
            } else {
                echo("ERROR");
