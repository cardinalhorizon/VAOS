...
class smartCARS
{
    ...
    public static function manuallogin($userid, $password, $sessionid)
    {
        global $dbConnection;
        $ret = array();
        $user = $userid;
    ...
    public static function automaticlogin($dbid, $oldsessionid, $sessionid)
    {
        $ret = array();
        global $dbConnection;
    ...
    public static function verifysession($dbid, $sessionid)
    {
        $ret = array();
        ...
        if ($res1['dbid'] != "") {
            ...
            $res = $stmt->fetch();
            $stmt->closeCursor();
            if ($res['id'] != "" && $res['retired'] == "0" && $res['confirmed'] != "0") {
                $ret['result'] = "SUCCESS";
                $ret['firstname'] = $res['first_name'];
                $ret['lastname'] = $res['last_name'];
                return $ret;
    ...
    public static function getpilotcenterdata($dbid)
    {
        global $dbConnection;
        $ret = array();
        $stmt = $dbConnection->prepare("SELECT * FROM " . TABLE_PREFIX . "users WHERE id = ?");
    ...
    public static function getairports($dbid)
    {
        global $dbConnection;
        $ret = array();
    ...
    public static function getaircraft($dbid)
    {
        global $dbConnection;
        $res = array();
        $stmt = $dbConnection->prepare("SELECT * FROM " . TABLE_PREFIX . "aircraft ORDER BY name");
        $stmt->execute();
    ...
    public static function getbidflights($dbid)
    {
        /*
        global $dbConnection;
    ...
    public static function bidonflight($dbid, $routeid)
    {
        global $dbConnection;
    ...
    public static function deletebidflight($dbid, $bidid)
    {
        global $dbConnection;
    ...
    public static function _helper_reorder_date_to_mmddyyyy($source)
    {
    ...
    public static function searchpireps($dbid, $departureicao, $arrivalicao, $startdate, $enddate, $aircraft, $status)
    {
        global $dbConnection;
        ...
        if ($departureicao != "" || $arrivalicao != "" || $startdate != "" || $enddate != "") {
            ...
            } elseif ($arrivalicao != "" && $departureicao == "") {
    ...
    public static function getpirepdata($dbid, $pirepid)
    {
        global $dbConnection;
    ...
    public static function searchflights($dbid, $departureicao, $mintime, $maxtime, $arrivalicao, $aircraft)
    {
        global $dbConnection;
    ...
    public static function createflight($dbid, $flightcode, $flightnumber, $ticketprice, $depicao, $arricao, $aircraft, $flighttype, $deptime, $arrtime, $flighttime, $route, $cruisealtitude, $distance)
    {
        global $dbConnection;
    ...
    public static function positionreport($dbid, $flightnumber, $latitude, $longitude, $magneticheading, $trueheading, $altitude, $groundspeed, $departureicao, $arrivalicao, $phase, $arrivaltime, $departuretime, $distanceremaining, $route, $timeremaining, $aircraft, $onlinenetwork)
    {
        global $dbConnection;
        $stmt = $dbConnection->prepare("SELECT * FROM " . TABLE_PREFIX . "users WHERE id = ?");
        $stmt->execute(array($dbid));
        ...
        $ret = $client->request('POST', VAOS_URL.'api/acars/smartCARS/positionreport', [
            'query' => [
                'format' => 'phpVMS'
            ],
            'form_params' => $fields
    ...
    public static function filepirep($dbid, $code, $flightnumber, $routeid, $bidid, $departureicao, $arrivalicao, $route, $aircraft, $load, $flighttime, $landingrate, $comments, $fuelused, $log)
    {
        global $dbConnection;
        $log = str_replace('[', '*[', $log);
