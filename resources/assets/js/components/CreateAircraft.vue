  ...
  <div>
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="form-group">
              ...
              <input
                class="form-control"
                v-model="aircraftData.icao"
                id="icao"
                type="text"
                @input="httpError = null"
                @keyup.tab="pullAircraftFromMaster"
                @keyup.enter="pullAircraftFromMaster"
              >
            </div>
            <div class="form-group">
    ...
    <div class="row">
      <div class="col-lg-6 col-md-12">
        ...
        <div class="card">
          <div class="card-body">
            <div class="alert alert-info" v-if="registrations.length === 0">
              To Add Aircraft, Click
              <i class="fa fa-plus"></i>
            </div>
              ...
              <div class="form-group">
                ...
                <select v-model="reg.hub_id" id="hub" class="custom-select">
                  ...
                  <template v-for="hub in hubs">
                    <option
                      v-bind:key="hub.pivot.id"
                      v-bind:value="hub.pivot.id"
                    >{{hub.icao}} | {{ hub.name }}</option>
                  </template>
                </select>
              </div>
              <div class="form-group" style="display: inline-grid">
                <button class="btn btn-danger mt-auto" @click="removeRegistration(index)">
        ...
        <form method="post" v-bind:action="'/admin/' + airline.id + '/fleet'">
          <input type="hidden" name="_token" v-bind:value="token">
          <input
            type="hidden"
            name="data"
            v-bind:value="JSON.stringify({ aircraftData, registrations, airline})"
          >
...
export default {
  ...
  methods: {
    pullAircraftFromMaster() {
      this.loading = true;
      axios
        .get("https://fsvaos.net/api/data/aircraft", {
          params: {
        ...
        .then(response => {
          this.loading = false;
          this.aircraftData.name = response.data.model_name;
