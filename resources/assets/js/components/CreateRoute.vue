  ...
  <div>
    <div class="row">
      <div class="col-lg-6 col-md-12">
        <div class="card">
          <div class="card-body">
            <h1>Airport
              <i class="fa fa-spinner fa-spin float-right" v-show="loading"></i>
            </h1>
            ...
            <div class="form-group">
              ...
              <input
                ...
                id="dicao"
                type="text"
                @input="httpError = null"
                @keyup.tab="depAptPull"
                @keyup.enter="depAptPull"
              >
            </div>
            <div class="form-group">
              ...
              <input
                ...
                type="text"
                @input="httpError = null"
                @keyup.tab="arrAptPull"
                @keyup.enter="arrAptPull"
              >
            </div>
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div v-if="depapt" class="form-group">
              <div>
                <span class="ls-title">ICAO:</span>
                {{ depapt.gps_code }}
              </div>
              <div>
                <span class="ls-title">IATA:</span>
                {{ depapt.iata_code }}
              </div>
              <div>
                <span class="ls-title">City:</span>
                {{ depapt.municipality }}
              </div>
              <div>
                <span class="ls-title">Country:</span>
                {{ depapt.iso_country }}
              </div>
            </div>
    ...
    <div class="row">
      <div class="col-lg-12 col-md-12">
        ...
        <div class="card">
          <div class="card-body">
            <div class="alert alert-info" v-if="routes.length === 0">To Add Routes, Click
              <i class="fa fa-plus"></i>
            </div>
            <div v-for="(route, index) in routes" :key="index">
              <div class="row">
                ...
                >
                  <label>
                    <template v-if="route.primary_group">Primary: {{ route.primary_group.icao }}</template>| Secondary:
                    <span v-for="e in route.aircraft_groups" v-bind:key="e.id">{{ e.icao + " "}}</span>
                  </label>
                  <br>
                  <button
                  ...
                  <div
                    class="dropdown-menu"
                    style="background: #333; padding: 5px;"
        ...
        <form method="post" v-bind:action="'/admin/' + airline.id + '/schedule'">
          ...
          <input
            type="hidden"
            name="data"
            v-bind:value="JSON.stringify({ airline, depapt, arrapt, routes })"
          >
...
export default {
  ...
  methods: {
    depAptPull() {
      this.loading = true;
      axios
        .get("https://fsvaos.net/api/data/airports", {
          params: {
            icao: this.depicao
          }
        })
